...
function makePassword(letterCount = 12, noSymbol = false, noNumber = false) {
  ...
  // let passCount = letterCount - passArray.filter(element => typeof element === 'string' || typeof element === 'number').reduce((count, element) => count + String(element).length, 0);
  ...
  do {
    ...
    switch (passCount) {
      ...
      default:
        if (passCount > 6) {
          passArray.push(sixLetter[Math.floor(Math.random() * fourLetter.length)]);
        }
        break;
    }
